...
from app import socketio, app , emit_all_messages
from app import BOT_NAME, BOT_PICTURE, MESSAGES_RECEIVED_CHANNEL
        ...
        self.keyerror_8ball_test_params =[
            {
                ...
                KEY_EXPECTED: {
                    KEY_BOT_ANSWER : "I'm over my limit for this API call"
                }
            }
        ]
...
def mocked_get_messages():
    return ([MockedMessage("text", "test.png", "test@test.com", "test-message"), MockedMessage("text", BOT_PICTURE, BOT_NAME, "bot-test-message")])

def mocked_get_user(email):
    return MockedUser(email, "test", "pic.png")

def mocked_emit_all_messages(channel):
    socketio.emit(MESSAGES_RECEIVED_CHANNEL, {'allMessages':[MockedMessage("text", "test.png", "test@test.com", "test-message")]})
